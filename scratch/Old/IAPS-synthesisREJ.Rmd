...
```{r tidy demographic data, cols.print = 2, message = FALSE, warning = FALSE}
...
genderCount <- dem.tab %>% count(gender) %>%
  ggplot(aes(reorder(gender, -n), y = n, fill = gender)) +
  geom_col() +
  labs(y = "Count",
       x = "Gender",
...
psychConditionCount <- dem.tab %>% count(condition_description) %>%
  drop_na() %>%
...
alcoholCount <- dem.tab %>% count(alcohol_current) %>%
  drop_na() %>%
  ggplot(aes(x = reorder(alcohol_current, -n), y = n, fill = alcohol_current)) +
  geom_col() +
  labs(y = "Count",
...
drugsCount <- dem.tab %>% count(drugs_current) %>%
  drop_na() %>%
  ggplot(aes(x = reorder(drugs_current, -n), y = n, fill = drugs_current)) +
...
smokeCount <- dem.tab %>% count(smoke_current) %>%
  drop_na() %>%
  ggplot(aes(x = reorder(smoke_current, -n), y = n, fill = smoke_current)) +
  geom_bar(stat = "identity") +
  geom_col() +
  labs(y = "Count",
       x = "Current Smokers",
...
(raceCount | ageCount | genderCount)/ (alcoholCount | drugsCount | smokeCount)/ psychConditionCount
...
```{r table 1 plot, warning = FALSE, message = FALSE}
...
```{r table 2 plot, warning = FALSE, message = FALSE}
