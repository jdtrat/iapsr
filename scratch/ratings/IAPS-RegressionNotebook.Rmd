...
```{r read data - original, message = FALSE, warning = FALSE}
...
netRec <- recipe(valmn ~ positive + negative + age + gender, data = train)

#Create a grid to tune the model for penalty and mixture (lambda and alpha) in
#an elastic net regression.
netGrid <- expand_grid(penalty = c(seq(0, 1, by = 0.01), seq(2,10, by = 1), seq(20, 100, by = 10)),
...
linReg <- lm(valmn ~ positive + negative + age + gender, data = train)
...
#Linear regression on training data, not cross validated
linRegInteract <- lm(valmn ~ positive + negative + age + gender + (positive * negative), data = train)
...
#create a recipe object -- simply describing the formula to fit for predicting
#the mean valence from the positive and negative ratings.
netRec_arousal <- recipe(aromn ~ positive + negative + age + gender, data = train)
...
#the mean valence from the positive and negative ratings. THEN add the
#interaction between positive and negative.
interactRec_arousal <- recipe(aromn ~ positive + negative + age + gender, data = train) %>%
  step_interact(terms = ~ positive:negative)
...
#Linear regression on training data, not cross validated
linReg_arousal <- lm(aromn ~ positive + negative + age + gender, data = train)
...
#Linear regression on training data, not cross validated
linRegInteract_arousal <- lm(aromn ~ positive + negative + age + gender + (positive * negative), data = train)
...
#create a recipe object -- simply describing the formula to fit for predicting
#the mean valence from the positive and negative ratings.
netRec_dominance <- recipe(dom1mn ~ positive + negative + age + gender, data = train)
...
#the mean valence from the positive and negative ratings. THEN add the
#interaction between positive and negative.
interactRec_dominance <- recipe(dom1mn ~ positive + negative + age + gender, data = train) %>%
  step_interact(terms = ~ positive:negative)
...
#Linear regression on training data, not cross validated
linReg_dominance <- lm(dom1mn ~ positive + negative + age + gender, data = train)
...
#Linear regression on training data, not cross validated
linRegInteract_dominance <- lm(dom1mn ~ positive + negative + age + gender + (positive * negative), data = train)
...
With our best models selected, we can now examine how well it fits on new subjects’ ratings. This just provides us an additional test data set for evaluating our model’s reliability. Similar to the original cohort, new subjects were asked to rank the IAPS images on a scale of one to ten in response to the following questions: “How positive does this make you feel?” and “How negative does this make you feel?” The data was parsed using the `iapsr` package we developed. The results of the models' fit can be seen below.

### Read in and Tidy New Cohort Data 

```{r set directories - new}

dirRatings <- '~/Desktop/IAPS/RJT/ratings/'
fileRatings <- dir_ls(dirRatings, glob = "*.txt.txt") #get the paths for the files that contain the string ".txt.txt".

```

```{r read data - new, warning = FALSE, message = FALSE}
...
Below, we can see a preview of the data we will test the model's fit on. This shows the first 10 rows for subject IAPS_B_01. Similar to the original cohort data, the age column is in years, gender is binary (0 for females 1 for males for use in regression analysis), IAPS column is the picture ID. The desc column is a description of the IAPS images. The positive and negative columns show the subject's ratings when asked "How positive does this image make you feel?" and "How negative does this image make you feel?". The remaining columns describe the mean and standard deviation for the image's standardized valence, arousal, and dominance, per the IAPS dataset.

```{r preview - new cohort, echo = FALSE}
...
```{r import iapsr subject 1, warning = FALSE, message = FALSE}

#read in the ratings data for subject 1 using the iapsr readRatings function
rawRJT_001 <- readRatings('IAPS_B.iaps_stream_rating.01.nosessionid.2020-07-16T10_27_19')

#process the raw ratings data using the iaspr processRatings data, pipe it into
#regSetup (also from our package!) and add subject name, age, and gender per
#redcap surveys. Reorder them because Jonathan wants them to be reordered. This
#will eventually be automated. Join that with the IAPS key for fitting
#regression.

RJT_001 <- processRatingsData(rawRJT_001) %>% 
  regSetup() %>%
  mutate(subject = "RJT_001",
         age = 31,
         gender = 0) %>%
  select(subject, age, gender, everything()) %>%
  left_join(keyIAPS)
...
```{r validate RJT_001 valence, warning = FALSE, message = FALSE}

#predict the valence for RJT_001 using the linReg model
val_RJT_001 <- linReg %>% 
  predict(RJT_001) %>% 
  bind_cols(RJT_001) %>% 
  rename(linRegPrediction = `...1`,
...
print("Model Fit Statistics for Predicting Valence using A Linear Regression:")
val_RJT_001Metrics <- metrics(data = val_RJT_001, truth = Actual, estimate = linRegPrediction) %>% 
  select(-.estimator) %>%
  rename(Metric = .metric,
         Estimate = .estimate)
val_RJT_001Metrics
...
```{r validate RJT_001 arousal, warning = FALSE, message = FALSE}

#predict arousal for RJT_001 using the linRegInteract_arousal model
aro_RJT_001 <- linRegInteract_arousal %>% 
  predict(RJT_001) %>% 
  bind_cols(RJT_001) %>% 
  rename(linRegInteractPrediction = `...1`,
         Actual = aromn)
...
print("Model Fit Statistics for Predicting Arousal using an Interact Linear Regression:")

aro_RJT_001Metrics <- metrics(data = aro_RJT_001, truth = Actual, estimate = linRegInteractPrediction) %>% 
  select(-.estimator) %>%
  rename(Metric = .metric,
         Estimate = .estimate)
aro_RJT_001Metrics
...
```{r validate RJT_001 dominance, warning = FALSE, message = FALSE}

# In order to get the elastic net, interact model to predict on new data, we
# must fit it to the original dataset first (in this case training, though we
# could do the finalData object as well). We save this object as
# finalFitInteract_dominance. We can then take that and use the predict function
# to get the new predictions.
finalFitInteract_dominance <- finalWFInteract_dominance %>% 
  fit(train)

#The variable importance for finalWFInteract_dominance fit. This is different
#than the variable importance from before, though the reason for that is unclear
#at the moment. I have some guesses, but I'm not sure.
#I'm commenting this out until I understand what's happening. I'm not worried
#about this, though. I have some suspicions for why it's acting a little funky.

# print("Model Beta Estimates for Elastic Net Regression - Interact:")
# finalFitInteract_dominance %>% 
#   pull_workflow_fit() %>% 
#   vi() %>%
#   select(-Sign)

#predict the fit on subject RJT_001
dom_RJT_001 <- finalFitInteract_dominance %>%
  predict(RJT_001) %>% 
  bind_cols(RJT_001) %>%
  rename(interactPrediction = .pred,
         Actual = dom1mn)
...
print("Model Fit Statistics for Predicting Dominance using an Interact Elastic Net Regression:")
dom_RJT_001Metrics <- metrics(data = dom_RJT_001, truth = Actual, estimate = interactPrediction) %>% 
  select(-.estimator) %>%
  rename(Metric = .metric,
         Estimate = .estimate)
dom_RJT_001Metrics
...
In summary, we used the models that fit valence, arousal, and dominance the best on previously collected data. When predicting on a new subject, we get an rmse of `r round(val_RJT_001Metrics %>% filter(Metric == "rmse") %>% pull(Estimate),3)`, `r round(aro_RJT_001Metrics %>% filter(Metric == "rmse") %>% pull(Estimate), 3)`, and `r round(dom_RJT_001Metrics %>% filter(Metric == "rmse") %>% pull(Estimate),3)`, for valence, arousal, and dominance, respectively. These compare with the test rmse when training the models of `r round(RMSEMetrics %>% slice(1) %>% pull(Estimate),3)` for valence, `r round(RMSEMetrics_arousal %>% slice(1) %>% pull(Estimate),3)` for arousal, and `r round(RMSEMetrics_dominance %>% slice(1) %>% pull(Estimate),3)` for dominance. 
