...
author: "Rachel Jones & Jonathan Trattner"
...
dirIAPS <- '~/Desktop/IAPS Synthesis/IAPS Data/' #set the directory for IAPS data
dirData <- '~/Desktop/IAPS Synthesis/Subject Data/' #set the directory for subject data
dirImageOrder <- '~/Desktop/IAPS Synthesis/Subject Image Order/' #set the directory for image order
dirRatings <- '~/Desktop/IAPS Synthesis/RJT/ratings/' #for new cohort
...
```{r read data ratings, message = FALSE, warning = FALSE}


#Read in the demographics data
demographics <- read_csv(file = "~/Desktop/IAPS Synthesis/Demographics/KLRIF_2018-07-09.csv")
...
newCohortDems <- read_csv(file = '~/Desktop/IAPS Synthesis/RJT/demographics/IAPSdems.csv') %>%
...
#gender data and we exclude). We also exclude subject 1069 who did not have any
#ratings (all 0s). The final data has 14 columns and 5640 rows and consists of
#120 positive and negative ratings from 47 subjects.
...
```{r read data dem stuff, warning = FALSE, message = FALSE}
#Read raw demographic data that includes 84 subjects. 
Dems <-read_csv(file = '~/Desktop/IAPS Synthesis/Demographics/SFA_demographics-downloaded-8-1-2020.csv')
...
demFinalData <- full_join(positiveRatings, negativeRatings, by = c("IAPS", "subject", "form")) %>%
  select(-c(order.x, order.y)) %>%
  full_join(keyIAPS, by = c("IAPS", "form")) %>%
  full_join(tidyDems, by = "subject") %>%
  drop_na() %>%
...
#Pivot dem.tab longer so the drugs stuff is all in one thing
dem.tab <- dem.tab %>% pivot_longer(cols = contains("_current")) %>%
  rename(drugs = name) %>%
  mutate(drugs = recode(drugs, 
                       "smoke_current" = "Cigarettes", 
                       "alcohol_current" = "Alcohol",
                       "drugs_current" = "Marijuana"),
         value = recode(value, `0` = "No", `1` = "Yes"))
...
myGGTheme <- ggplot2::theme(plot.title = ggplot2::element_text(hjust=0.5, face = "bold.italic", size=12), #plot title aesthetics
                   plot.subtitle = ggplot2::element_text(hjust = 0.5, face = "bold", size = 8), #plot subtitle aesthetics
                   axis.title.x = ggplot2::element_text(size = 8, color= "black", face = "bold"), #x axis title aesthetics
                   axis.title.y = ggplot2::element_text(size = 8, color= "black", face = "bold"), #y axis title aesthetics
...
raceCount <- dem.tab %>% count(subject, race) %>%
  drop_na() %>%
  distinct(subject, race) %>%
  count(race) %>%
  mutate(race = fct_reorder(race, desc(n)),
         percent = round((n / 47),3) * 100) %>%
  ggplot(aes(x = race, y = n, fill = race)) +
  geom_col() +
  geom_text(position = position_dodge(width = 0.9), aes(label = glue::glue("{percent}%"), vjust = -0.5)) +
  labs(y = "Count",
       x = "Race",
...
ageCount <- dem.tab %>% group_by(subject) %>% distinct(age) %>%
  ungroup() %>%
  select(age) %>%
...
genderCount <- dem.tab %>% count(subject, gender) %>%
  distinct(subject, gender) %>% 
  count(gender) %>%
  mutate(percent = round((n / 47),3) * 100,
...
psychCondtionCount <- dem.tab %>% 
  count(subject, condition_description) %>%
  mutate(condition_description = case_when(condition_description == "Anxiety" ~ "Anxiety",
                                           str_detect(condition_description, "Depression") ~ "Depression\nComorbid", 
                                           TRUE ~ "None")) %>%
  drop_na() %>%
  distinct(subject, condition_description) %>%
  count(condition_description) %>%
  mutate(percent = round((n / 47),3) * 100,
         condition_description = fct_reorder(condition_description, desc(n))) %>%
...
drugsPlot <- dem.tab %>% count(drugs, value) %>% 
  mutate(percent = round((n / 47),3) * 100) %>% 
  ggplot(aes(x = drugs, y = n, fill = value)) +
  geom_col(position = position_dodge2(preserve = "single")) +
  geom_text(position = position_dodge2(width = 0.9, preserve = "single"), aes(label = glue::glue("{percent}%"), vjust = -0.5, hjust = 0.36)) +
  labs(title = "Drug Use",
       subtitle = "Original Cohort",
       x = "Drug Type",
       y = "Count",
...
(raceCount | ageCount | genderCount)

(drugsPlot + psychCondtionCount)
...
```{r preview, cols.print = 7, echo = FALSE, fig.cap = "Final Data Blah aASf"}
...
finalWFInteract <- finalize_workflow(wfInteract, parameters = bestMetricsInteract)
...
The choice game was a 150-trial two-choice probabilistic reward task with emotionally-stimulating images from the IAPS database used as reinforcers. These reinforcers were split up into six different reinforcer groups based on the Z scores of the old cohort's average positive rating for each image. Each trial began with presentation of two icons on a computer screen, and participants were required to choose between them using a Logitech gaming controller. A total of 58 icons were randomly selected to be in one of six groups, with six unique icons per group (Table 1). Within each group, icons held fixed probabilities of displaying positive, neutral, or negative images as outcomes. The game was divided into three phases that the icon groups corresponded to, with 25 trails in phase one, 50 trails in phase 2, and 75 trials in phase 3 (Table 2). 


```{r table 1, include = TRUE, warning = FALSE, message = FALSE}
IAPSgroups1 <- read_csv('~/Desktop/IAPS Synthesis/Figure Info/IAPSgroups.csv')

flextable(head(IAPSgroups1)) %>%
...
```{r table 2, include = TRUE, warning = FALSE, message = FALSE}
IAPSphases1 <- read_csv('~/Desktop/IAPS Synthesis/Figure Info/IAPSphases.csv')

flextable(head(IAPSphases1)) %>%
...
![Figure 1: Schematic of one trial in IAPS choice game.](TrialGraphic.png)
...
choiceFiles <- dir_ls('~/Desktop/IAPS Synthesis/RJT/choice/', glob = "*.txt.txt") #get the paths for the files that contain the string ".txt.txt". For new cohort.
...
```{r tidy data - choice task, cols.print = 7, warning = FALSE, message = FALSE}
